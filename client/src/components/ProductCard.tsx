...
const ProductCard = ({product}:Props) => {
    ...
    onClick={()=>navigate(`/products/${product._id}`)}>
        ...
        <div className="relative aspect-square overflow-hidden">
            ...
            className="w-full h-full object-cover p-4 group-hover:p-2 tansition-all duration-300"/>
        {/* Badges*/}
            <div className="absolute top-3 left-3 flex flex-wrap gap-1.5">
                {product.discount > 0 &&<span className="px-2 py-0.5 text-[10px] font-semibold
                uppercase bg-app-orange text-white rounded-full">{product.discount}% OFF
                </span>}
